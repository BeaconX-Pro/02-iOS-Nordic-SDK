...
@implementation MKBXPSlotDataAdopter

+ (mk_bxp_slotFrameType)fetchSlotFrameType:(NSString *)type {
    if (!ValidStr(type) || [type isEqualToString:@"ff"]) {
        return mk_bxp_slotFrameTypeNull;
    }
    if ([type isEqualToString:@"00"]) {
        return mk_bxp_slotFrameTypeUID;
    }
    if ([type isEqualToString:@"10"]) {
        return mk_bxp_slotFrameTypeURL;
    }
    if ([type isEqualToString:@"20"]) {
        return mk_bxp_slotFrameTypeTLM;
    }
    if ([type isEqualToString:@"40"]) {
        return mk_bxp_slotFrameTypeInfo;
    }
    if ([type isEqualToString:@"50"]) {
        return mk_bxp_slotFrameTypeBeacon;
    }
    if ([type isEqualToString:@"60"]) {
        return mk_bxp_slotFrameTypeThreeASensor;
    }
    if ([type isEqualToString:@"70"]) {
        return mk_bxp_slotFrameTypeTHSensor;
    }
    return mk_bxp_slotFrameTypeNull;
}
