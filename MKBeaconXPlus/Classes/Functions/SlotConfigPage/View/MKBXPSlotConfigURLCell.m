...
@implementation MKBXPSlotConfigURLCellModel
@end

@interface MKBXPSlotConfigURLCell ()
...
@implementation MKBXPSlotConfigURLCell

+ (MKBXPSlotConfigURLCell *)initCellWithTableView:(UITableView *)tableView {
    MKBXPSlotConfigURLCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKBXPSlotConfigURLCellIdenty"];
    if (!cell) {
        cell = [[MKBXPSlotConfigURLCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKBXPSlotConfigURLCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKBXPSlotConfigURLCellModel *)dataModel {
    _dataModel = nil;
...
- (void)loadUrlInfo:(NSData *)advData{
    ...
    for (int i = 0; i < advData.length - 3; i++) {
        url = [url stringByAppendingString:[MKBXPSlotDataAdopter getEncodedString:*(data + i + 3)]];
    }
    self.urlTypeLabel.text = [MKBXPSlotDataAdopter getUrlscheme:*(data+2)];
...
- (NSDictionary *)getLegitimateUrl:(NSString *)header urlContent:(NSString *)urlContent{
    ...
    NSString *expansion = [MKBXPSlotDataAdopter getExpansionHex:[@"." stringByAppendingString:[urlList lastObject]]];
    ...
    return @{
        ...
        @"result":@{
                ...
                @"params":@{
                        @"frameType":bxp_slotConfig_urlData,
                        @"urlHeader":header,
...
- (UIImageView *)leftIcon{
    if (!_leftIcon) {
        ...
        _leftIcon.image = LOADICON(@"MKBeaconXPlus", @"MKBXPSlotConfigUIDCell", @"bxp_slot_advContent.png");
    }
    return _leftIcon;
}
