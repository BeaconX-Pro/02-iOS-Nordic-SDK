...
@implementation MKBXPTriggerTemperatureViewModel
@end

@interface MKBXPTriggerTemperatureView ()
...
@implementation MKBXPTriggerTemperatureView
...
- (void)sliderValueChanged {
    ...
    [self updateNoteMsg];
    if ([self.delegate respondsToSelector:@selector(bxp_triggerTemperatureThresholdValueChanged:)]) {
        [self.delegate bxp_triggerTemperatureThresholdValueChanged:[value floatValue]];
    }
}
...
- (void)startLabelPressed {
    ...
    [self updateNoteMsg];
    if ([self.delegate respondsToSelector:@selector(bxp_triggerTemperatureStartStatusChanged:)]) {
        [self.delegate bxp_triggerTemperatureStartStatusChanged:YES];
    }
}
...
- (void)stopLabelPressed {
    ...
    [self updateNoteMsg];
    if ([self.delegate respondsToSelector:@selector(bxp_triggerTemperatureStartStatusChanged:)]) {
        [self.delegate bxp_triggerTemperatureStartStatusChanged:NO];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKBXPTriggerTemperatureViewModel *)dataModel {
    _dataModel = nil;
...
- (void)updateNoteMsg {
    NSString *msg = [NSString stringWithFormat:@"*The Beacon will %@ when the temperature is %@ %@",(self.start ? @"start to broadcast" : @"stop broadcasting"),(self.dataModel.above ? @"above" : @"below"),self.sliderValueLabel.text];
    self.noteMsgLabel.text = msg;
}

- (void)updateSlectedIcon {
    self.startIcon.image = (self.start ? LOADICON(@"MKBeaconXPlus", @"MKBXPTriggerTemperatureView", @"bxp_slot_paramsConfig_selectedIcon.png") : LOADICON(@"MKBeaconXPlus", @"MKBXPTriggerTemperatureView", @"bxp_slot_paramsConfig_unselectedIcon.png"));
    self.stopIcon.image = (self.start ? LOADICON(@"MKBeaconXPlus", @"MKBXPTriggerTemperatureView", @"bxp_slot_paramsConfig_unselectedIcon.png") : LOADICON(@"MKBeaconXPlus", @"MKBXPTriggerTemperatureView", @"bxp_slot_paramsConfig_selectedIcon.png"));
}
...
- (UIImageView *)startIcon {
    if (!_startIcon) {
        ...
        _startIcon.image = LOADICON(@"MKBeaconXPlus", @"MKBXPTriggerTemperatureView", @"bxp_slot_paramsConfig_selectedIcon.png");
    }
    return _startIcon;
}
...
- (UIImageView *)stopIcon {
    if (!_stopIcon) {
        ...
        _stopIcon.image = LOADICON(@"MKBeaconXPlus", @"MKBXPTriggerTemperatureView", @"bxp_slot_paramsConfig_unselectedIcon.png");
    }
    return _stopIcon;
}
