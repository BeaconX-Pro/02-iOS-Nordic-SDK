//
//  MKBXPStorageTriggerHumidityView.m
...
@interface MKBXPStorageTriggerHumidityView ()
...
@implementation MKBXPStorageTriggerHumidityView
...
- (void)updateHumidityValue:(NSString *)humidity {
    if (!ValidStr(humidity)) {
        return;
    }
    [self.valueButton setTitle:humidity forState:UIControlStateNormal];
...
- (void)processSelectMethod:(NSInteger)index {
    if (index == 0) {
        self.noteLabel.text = @"*The device stores all sampled T&H data.";
    }else {
        self.noteLabel.text = [NSString stringWithFormat:@"*The device stores T&H data when the humodity changed ≥ %@%@.",self.dataList[index],@"%RH"];
    }
    [self setNeedsLayout];
}
...
- (UIButton *)valueButton {
    if (!_valueButton) {
        ...
        [_valueButton setTitle:@"0.0" forState:UIControlStateNormal];
