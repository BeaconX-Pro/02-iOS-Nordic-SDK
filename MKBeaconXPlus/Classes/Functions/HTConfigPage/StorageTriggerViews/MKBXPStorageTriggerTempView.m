...
@interface MKBXPStorageTriggerTempView ()
...
@implementation MKBXPStorageTriggerTempView
...
- (void)updateTemperateure:(NSString *)temperateure {
    if (!ValidStr(temperateure)) {
        return;
    }
    [self.valueButton setTitle:temperateure forState:UIControlStateNormal];
...
- (void)processSelectMethod:(NSInteger)index {
    if (index == 0) {
        ...
        self.noteLabel.text = [NSString stringWithFormat:@"*The device stores T&H data when the temperateure changed ≥ %@℃.",self.dataList[index]];
    }
    [self setNeedsLayout];
}
