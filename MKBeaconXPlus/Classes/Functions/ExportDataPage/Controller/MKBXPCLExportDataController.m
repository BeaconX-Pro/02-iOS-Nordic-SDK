...
@interface MKBXPCLExportDataController ()
...
@property (nonatomic, strong)MKBXPRecodeHistoryHudView *hudView;

@property (nonatomic, assign)NSInteger totalNum;

@end

@implementation MKBXPCLExportDataController

- (void)dealloc {
    NSLog(@"MKBXPCLExportDataController销毁");
    [[NSNotificationCenter defaultCenter] removeObserver:self
                                                    name:mk_bxp_cl_receiveHTDataNotification
                                                  object:nil];
    [[MKBXPCentralManager shared] notifyBXPCLHTData:NO];
    ...
    if (self.syncButton.selected) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(receiveRecordHTData:)
                                                     name:mk_bxp_cl_receiveHTDataNotification
                                                   object:nil];
        [[MKBXPCentralManager shared] notifyBXPCLHTData:YES];
...
- (void)switchButtonPressed {
    ...
    UIImage *icon = (self.switchButton.selected ? LOADICON(@"MKBeaconXPlus", @"MKBXPCLExportDataController", @"bxp_exportHT_curveSelected.png") : LOADICON(@"MKBeaconXPlus", @"MKBXPCLExportDataController", @"bxp_exportHT_tableSelected.png"));
...
- (void)receiveRecordHTData:(NSNotification *)note {
    ...
    NSString *msg = [NSString stringWithFormat:@"Reading data ...... ,update %ld records",(long)self.totalNum];
    [self.hudView showMsg:msg];
    if (total <= (index + 1)) {
        //接收完成
        [[NSNotificationCenter defaultCenter] removeObserver:self];
        [self performSelector:@selector(showComplete) withObject:nil afterDelay:2.f];
    }
}
...
- (void)deleteRecordDatas {
    ...
    [self.synIcon.layer removeAnimationForKey:@"synIconAnimationKey"];
    [[MKBXPCentralManager shared] notifyBXPCLHTData:self.syncButton.selected];
    self.syncLabel.text = @"Sync";
...
- (void)reloadHTDatas:(NSDictionary *)dic {
...
- (void)readDatasFromLocal {
    ...
    [MKBXPInterface bxp_readHundredHistoryDataWithSucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (UIImageView *)synIcon {
    if (!_synIcon) {
        ...
        _synIcon.image = LOADICON(@"MKBeaconXPlus", @"MKBXPCLExportDataController", @"bxp_threeAxisAcceLoadingIcon.png");
    }
    return _synIcon;
}
...
- (UIButton *)switchButton {
    if (!_switchButton) {
        ...
        [_switchButton setImage:LOADICON(@"MKBeaconXPlus", @"MKBXPCLExportDataController", @"bxp_exportHT_tableSelected.png") forState:UIControlStateNormal];
...
- (UIButton *)deleteButton {
    if (!_deleteButton) {
        ...
        [_deleteButton setImage:LOADICON(@"MKBeaconXPlus", @"MKBXPCLExportDataController", @"bxp_slotExportDeleteIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)exportButton {
    if (!_exportButton) {
        ...
        [_exportButton setImage:LOADICON(@"MKBeaconXPlus", @"MKBXPCLExportDataController", @"bxp_slotExportEnableIcon.png") forState:UIControlStateNormal];
...
- (MKBXPRecodeHistoryHudView *)hudView {
    if (!_hudView) {
        _hudView = [[MKBXPRecodeHistoryHudView alloc] init];
    }
    return _hudView;
}
