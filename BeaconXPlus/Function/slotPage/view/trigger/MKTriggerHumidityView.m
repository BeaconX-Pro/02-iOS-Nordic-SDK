...
@interface MKTriggerHumidityView ()
...
@implementation MKTriggerHumidityView
...
- (void)sliderValueChanged {
    self.sliderValueLabel.text = [NSString stringWithFormat:@"%.f%@",self.humiditySlider.value,@"%"];
    [self updateNoteMsg];
}
...
- (void)updateAbove:(BOOL)above start:(BOOL)start {
    self.above = above;
    self.start = start;
    self.msgLabe.attributedText = (above ? [MKAttributedString getAttributedString:@[@"Humidity Above",@"   (0%~100%)"] fonts:@[MKFont(13.f),MKFont(11.f)] colors:@[DEFAULT_TEXT_COLOR,RGBCOLOR(223, 223, 223)]] : [MKAttributedString getAttributedString:@[@"Humidity Below",@"   (0%~100%)"] fonts:@[MKFont(13.f),MKFont(11.f)] colors:@[DEFAULT_TEXT_COLOR,RGBCOLOR(223, 223, 223)]]);
    [self updateNoteMsg];
    [self updateSlectedIcon];
}
...
- (void)updateNoteMsg {
    NSString *msg = [NSString stringWithFormat:@"The device will %@ advertising if the humidity is %@ %@",(self.start ? @"start" : @"stop"),(self.above ? @"above" : @"below"),self.sliderValueLabel.text];
...
- (UILabel *)msgLabe {
    if (!_msgLabe) {
        ...
        _msgLabe.attributedText = [MKAttributedString getAttributedString:@[@"Humidity Above",@"   (0%~100%)"] fonts:@[MKFont(13.f),MKFont(11.f)] colors:@[DEFAULT_TEXT_COLOR,RGBCOLOR(223, 223, 223)]];
    }
    return _msgLabe;
}
